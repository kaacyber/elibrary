...
{	
    ...
    function jenis_pengadaan($where=array('kd_tender !='=>''),$order="nama ASC")
    {
		$d=$this->m_db->get_data('tender',$where,$order);
		return $d;
	}

	function tender_info($beaID,$output)
	{
		$s=array(
		'kd_tender'=>$beaID,
		);
		$item=$this->m_db->get_row('tender',$s,$output);
		return $item;
	}

	function tender_data($where=array('id !='=>''),$order="id ASC")
    {
		$d=$this->m_db->get_data('daf_kriteria',$where,$order);
		return $d;
	}
    
    function beasiswa_data($where=array('id !='=>''),$order="id ASC")
    {
		$d=$this->m_db->get_data('daf_kriteria',$where,$order);
		return $d;
	}
