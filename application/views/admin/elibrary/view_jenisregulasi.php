	...
	<tbody>
		...
		if(!empty($data))
		{
			...
			<tr>
				...
				<td ><?=$row->id;?></td>
				<td ><?=$row->jenis_regulasi;?></td>
				<td>
					<a onclick="return confirm('Yakin ingin menghapus user ini?');" href="<?=base_url(akses().'/elibrary/deletejenisregulasi');?>?id=<?=$row->id;?>" class="btn btn-xs btn-danger">Delete</a>
				</td>
			</tr>
			<?php
			}
