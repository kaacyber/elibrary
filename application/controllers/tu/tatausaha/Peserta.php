...
class Peserta extends CI_Controller
{
    ...
    function __construct()
    {
		...
		$this->load->model('pelamar_model','mod_siswa');
		$this->load->model('kriteria_model','mod_kriteria');
		$s=array(
		'user_id'=>user_info('user_id'),
		);
		$this->x_wali=$this->m_db->get_row('user_reg',$s,'wali_kelas_id');
		//$this->x_kelas=$this->m_db->get_row('user_reg','','kelas');
		//$this->x_jurusan=$this->m_db->get_row('user_reg','','jurusan');
		$this->x_nuptk=$this->m_db->get_row('user_reg',$s,'nuptk');
    }
    ...
    function index()
    {
    	...
    	$sql="SELECT peserta.peserta_id,pelamar.pelamar_id,pelamar.nisn,pelamar.nama,beasiswa.judul,peserta.status,beasiswa.tahun FROM (peserta LEFT JOIN pelamar ON peserta.siswa_id = pelamar.pelamar_id) LEFT JOIN beasiswa ON peserta.beasiswa_id = beasiswa.beasiswa_id".$s;
